...
int main() {
  ...
  do {
    ...
    { 
      printf("set %d\n", i);
      asm volatile ("fence");
      asm volatile ("custom0 %0, %[x], %[y], 3"  :  "=r"(ld_result_1) : [x]"r"(0), [y]"r"(&a[i-1]));
      printf("data loaded from address %d is %d\n", &a[i-1], ld_result_1);
      
      asm volatile ("fence");
      asm volatile ("custom0 %0, %[x], %[y], 3"  :  "=r"(ld_result_2) : [x]"r"(0), [y]"r"(&b[i-1]));
      printf("data loaded from address %d is %d\n", &b[i-1], ld_result_2);
      
      asm volatile ("fence");
      asm volatile ("custom0 %0, %[x], %[y], 1"  :  "=r"(add_result) : [x]"r"(ld_result_1), [y]"r"(ld_result_2));
      printf("add %d to %d, the result is %d\n", ld_result_1, ld_result_2, add_result);
      
      asm volatile ("fence");
      asm volatile ("custom0 %0, %[x], %[y], 2"  :  "=r"(store_result) : [x]"r"(add_result), [y]"r"(&c[i-1]));
